/**
 * CSV Cluster Worker Entry Point
 * This file is executed when a cluster worker is forked for CSV processing
 */
...
class CSVClusterProcessor {
  ...
  async processCSV(filePath: string): Promise<ProcessingResult> {
    ...
    try {
      logger.info(`Starting CSV processing in cluster worker ${process.pid}`)

      // Initialize database connection
      await initializeDatabase()
      logger.info('Database connected in cluster worker')
      ...
      logger.info('CSV processing completed in cluster worker', result)
      return result
    } catch (error) {
      logger.error('Error in CSV cluster processor:', error)
      result.errors.push(
...
// This file should only be run as a cluster worker
if (cluster.isWorker) {
  const processor = new CSVClusterProcessor()

  logger.info(`CSV Cluster worker ${process.pid} started`)

  // Listen for messages from master process
  process.on('message', (message: { filePath: string }) => {
    void (async () => {
      try {
        logger.info(
          `Cluster worker ${process.pid} received file: ${message.filePath}`
        )
        const result = await processor.processCSV(message.filePath)

        // Send result back to master
        process.send?.({ success: true, result })
        logger.info(`Cluster worker ${process.pid} completed processing`)
      } catch (error) {
        logger.error(`Cluster worker ${process.pid} error:`, error)
        process.send?.({
          success: false,
  ...
  process.on('SIGTERM', () => {
    logger.info(
      `CSV Cluster worker ${process.pid} received SIGTERM, shutting down`
    )
  ...
  process.on('SIGINT', () => {
    logger.info(
      `CSV Cluster worker ${process.pid} received SIGINT, shutting down`
    )
    // eslint-disable-next-line node/no-process-exit
    process.exit(0)
  })
} else {
  logger.error('CSV Cluster worker should only be run as a cluster worker')
  // eslint-disable-next-line node/no-process-exit
  process.exit(1)
}
